...
async function checkAndUpdateUserName(userid, username) {
  let user = await ctx.database.get("codegang_jf", { userid });
  if (user.length == 0) {
    await ctx.database.create("codegang_jf", { userid, username, jf: 0 });
  } else {
    if (user[0].username != username) {
      await ctx.database.set("codegang_jf", { userid }, { username });
    }
  }
}
__name(checkAndUpdateUserName, "checkAndUpdateUserName");
...
async function getfortunev2(userid) {
  if (!userid) {
...
__name(getfortunev2, "getfortunev2");
...
var Config = import_koishi2.Schema.intersect([
  ...
  import_koishi2.Schema.object({
    picApi: import_koishi2.Schema.string().default("https://t.alcy.cc/pc/").description("图片API地址"),
    setuApi: import_koishi2.Schema.string().default("https://api.lolicon.app/setu/v2").description("涩图API地址")
  }).description("图片配置"),
  import_koishi2.Schema.object({
...
async function apply(ctx2, cfg) {
  ctx2.model.extend("codegang_jf", {
    id: "unsigned",
    userid: "string",
    username: "string",
    jf: "integer",
  ...
  dajf.init(ctx2.database, cfg);
  ncm.init(ctx2.http, cfg);
  init(ctx2);
  ctx2.command("test").action(async ({ session }) => {
    if (ctx2.mail) {
      session.send((await ctx2.mail.list(session.userId)).data.length.toString());
    }
  });
  ctx2.command("积分排行").alias("排行").action(async ({ session }) => {
    ...
    topUsers.forEach((item, index) => {
      ...
      msg += `${index + 1}. ${name2}—${item.jf}
`;
    });
    session.send(msg);
  });
  ctx2.command("我的积分").alias("查询积分").alias("积分查询").alias("积分").alias("jf").action(async ({ session }) => {
    (0, import_koishi3.sleep)(cfg.delay);
    session.send(`你的积分是${await dajf.get(session.userId)}`);
  });
  ctx2.command("签到").alias("qd").action(async ({ session }) => {
    checkAndUpdateUserName(session.userId, session.username);
...
${fortune}${mail}`);
    ...
    let hitokoto = await getHitokoto(ctx2.http);
    if (cfg.isdev) {
      return `【灰度测试】签到成功，你获得了${upjf}积分` + mail + (usertype == 0 ? "\n这是你首次签到哦" : "") + `
${hitokoto}
${fortune}
${img}`;
    }
...
${img}`);
  ...
  ctx2.command("积分商城").alias("积分商店").action(async ({ session }) => {
    (0, import_koishi3.sleep)(cfg.delay);
    session.send(cfg.menu);
  });
  ctx2.command("兑换 <thing> [arg1]").action(async ({ session }, thing, arg1) => {
    switch (true) {
      case thing == null: {
        (0, import_koishi3.sleep)(cfg.delay);
      ...
      case (thing == "1" || thing == "一言"): {
        ...
        session.send(await getHitokoto(ctx2.http));
        break;
      }
      ...
      case (thing == "3" || thing == "涩图"): {
        ...
        try {
          let url = await ctx2.http.get("https://api.lolicon.app/setu/v2");
