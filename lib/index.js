...
async function getfortune(http, userid) {
  try {
    ...
    if (row) {
      ...
      let res2 = { index: "", star: "", sign: "", res: "" };
      res2.index = row.match(/<dt>运情总结：<\/dt>\s*<dd>([\s\S]*?)<\/dd>/)[1];
      res2.star = row.match(/<dt>幸运星：<\/dt>\s*<dd>([\s\S]*?)<\/dd>/)[1];
      res2.sign = row.match(/<dt>签文：<\/dt>\s*<dd>([\s\S]*?)<\/dd>/)[1];
      res2.res = row.match(/<dt>解签：<\/dt>\s*<dd>([\s\S]*?)<\/dd>/)[1];
      return `今日运势：${res2.index}${res2.star}
${res2.sign}
${res2.res}`;
    } else {
