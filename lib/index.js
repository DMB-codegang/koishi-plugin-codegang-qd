...
`;
  ...
  ctx2.command("签到").alias("qd").action(async ({ session }) => {
    ...
    let user = await ctx2.database.get("codegang_jf", { userid: session.userId });
    if (user.length == 0) {
      await ctx2.database.create("codegang_jf", { userid: session.userId, username: session.username, jf: 0 });
    } else {
      if (user[0].username != session.username) {
        await ctx2.database.set("codegang_jf", { userid: session.userId }, { username: session.username });
      }
    }
